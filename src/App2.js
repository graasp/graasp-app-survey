...
const App2 = () => {
  const total = questions.length * students.length;
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [stdObjects, setStdObjects] = useState([]);
  const [rows, setRows] = useState([]);
  const [columns, setColumns] = useState([
    { id: "question", label: "Did the Team Member...", minWidth: 170 },
  ]);

  const createData = (id, question) => {
    let obj = { id, question };
  ...
  useEffect(() => {
    const newww = students.map((student) => ({
      name: student,
      // content: <CustomCheckbox id={student} total={total} />,
      content:'hii',
    }));
    setStdObjects(newww);
    console.log('newww',newww)

    const rrr = questions.map((question, index) =>
      // rows.push(createData(uuidv4(), index + 1 + ". " + question, stdObjects))
      createData(uuidv4(), index + 1 + ". " + question, newww)
    );
    setRows([...rows, ...rrr]);

    const sss = newww.map((student) =>
      // columns.push({ id: student.name, label: student.name, minWidth: 100 })
      ({ id: student.name, label: student.name, minWidth: 100 })
    );

    setColumns([...columns, ...sss]);
